...
import { DashboardContainerOutput, PanelState } from '../../../dashboard_embeddable/public';
// @ts-ignore
...
export class DashboardStateManager {
  ...
  private appState: DashboardAppState;
  ...
  public handleDashboardEmbeddableChanges(dashboardOutput: DashboardContainerOutput) {
    ...
    this.getPanels().forEach(savedDashboardPanel => {
      if (dashboardOutput.panels[savedDashboardPanel.panelIndex] !== undefined) {
    ...
    Object.values(dashboardOutput.panels).forEach(panelState => {
      if (savedDashboardPanelMap[panelState.embeddableId] === undefined) {
        throw new Error(
          'At this time, adding new panels from the dashboard embeddable is not supported. It must come from the dashboard app'
        );
      }
  ...
  public removePanel(panelIndex: string) {
    _.remove(this.getPanels(), (panel: SavedDashboardPanel) => {
